...
CREATE TABLE IF NOT EXISTS `法人证券账户` (
  ...
  `法人注册登记号码` char(9) COLLATE utf8_unicode_ci NOT NULL,
...
*/
    ...
    {
        $query = $this->db->query("SELECT 股票账户号码 FROM 法人证券账户 WHERE 股票账户号码='$sid_in'");
        if($query->num_rows()>0)
            return true;
        else
            return false;      
    ...
    {
        $query = $this->db->query("SELECT 法人身份证号码 FROM 法人证券账户 WHERE 股票账户号码='$sid_in'")
        $row = $query->result();
        return $row[0]->法人身份证号码;
    }
