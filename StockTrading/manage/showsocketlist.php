      ...
      <div class="row row-offcanvas row-offcanvas-right">
        ...
        <div class="col-xs-8 col-sm-8">
          ...
          <table class="table" id="stocktable">
              ...
              <?php 
               include("conn.php");
                $stock=mysqli_query($conn,"select * from admin_stock where admin_id=".$id);

                while($row = mysqli_fetch_array($stock))
                {
                  $stockinfo=mysqli_query($conn,"select * from stock where uid=".$row['uid']);
                  $myrow=mysqli_fetch_array($stockinfo);
                  echo "<tr>";
                  echo "<td>" . $myrow['uid'] . "</td>";
                  echo "<td>" . $myrow['name'] . "</td>";
                  echo "<td>
                            <button type='button' class='btn btn-info'>
                            <a href='deleteband.php?uid=".$myrow['uid']."&adminid=".$id."'>删除绑定</a>
                            </button>
                        </td>
                          ";
                  echo "</tr>";
          ...
          <div class="form-group">
                <div class="col-sm-offset-2 col-sm-1">
                  <a href="#" class="btn btn-lg btn-success" data-toggle="modal" data-target="#addstockadmin">绑定股票</a>
                </div>
            ...
            <div class="modal fade" id="addstockadmin" tabindex="-1" role="dialog" aria-labelledby="basicModal" aria-hidden="true">
                <div class="modal-dialog">
                  <div class="modal-content">
                    <div class="modal-header">
                      <button type="button" class="close" data-dismiss="modal" aria-hidden="true">&times;</button>
                      <h4 class="modal-title" id="myModalLabel">绑定股票和该管理员</h4>
                    </div>
                    <div class="modal-body">
                       <form role="form" method="post" action="addstockadmin.php?">
                        <!--<div class="form-group">
                          <label for="id">管理员id</label>
                        ...
                        <div class="control-group">
                           ...
                           <input type="text" class="form-control" name="adminid" id="adminid" value="<?php echo $id?>"  readonly="readonly" >
                         </div>
                        <!--<div class="form-group">
                          <label for="uid">股票id</label>
                          <input type="text" class="form-control" name="uid" id="uid">
                        </div>-->
						<?php
							//$res1=mysqli_query($conn,"select * from admin_stock");
							$res2=mysqli_query($conn,"SELECT uid FROM stock WHERE NOT EXISTS ( SELECT uid FROM admin_stock WHERE stock.uid = admin_stock.uid)");
